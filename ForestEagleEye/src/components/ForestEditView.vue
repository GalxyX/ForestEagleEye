<template>
  ...
  <div style="margin-top:20px; margin-left: 30px;margin-right: 30px;">
    <!-- 变量信息表单 -->
    <el-descriptions title="变量信息" direction="vertical" :column="1"></el-descriptions>
    <div>
      <div>
        <h1>编辑气候概况</h1>
        <div style="display: flex; align-items: center; gap: 10px;">
          <h2>
            <el-alert title="温馨提示：点击按钮获取本林区的最新气象数据，并更新到林上鹰眼数据库" type="warning" :closable="false"
              style="font-size: small;">
            </el-alert>
          </h2>

          <el-button type="success" plain style="width: 80px;" v-if="!showLoading"
            @click="fetchWeatherData">获取数据</el-button>
          <el-button type="success" plain loading style="width: 80px;" v-else disabled>Loading</el-button>
        </div>
        <el-descriptions direction="vertical" :column="5" border v-if="showWeatherDataBox">
      ...
      <div>
        <div style="display: flex; align-items: center; margin-top: 10px;">
          <h1>编辑森林简介</h1>
          <img v-if="!isEditIntro" @click="toggleEditIntro" src="../assets/icon-pencil2.png"
      ...
      <div>
        <h1>编辑资源分布</h1>
        <h2>
          <el-alert title="温馨提示：按照参考格式上传数据文件，林上鹰眼会自动帮您保存到数据库中" type="warning" :closable="false"
            style="font-size: small;">
          </el-alert>
        </h2>

        <el-descriptions direction="vertical" style="margin-bottom: 10px;" :column="5" border>
        ...
        <el-upload ref="uploadResource" action="http://127.0.0.1:5000/uploadResourceFile" :limit="1"
          :on-exceed="handleExceedResource" :auto-upload="false" :data="uploadForest">
          <template #trigger>
      ...
      <div>
        ...
        <el-upload ref="uploadDisaster" action="http://127.0.0.1:5000/uploadDisasterFile" :limit="1"
          :on-exceed="handleExceedDisaster" :auto-upload="false" :data="uploadForest">
          <template #trigger>
            <el-button type="success" plain>选择文件</el-button>
            <h3 style="margin-left: 10px;">
              您只能上传1个.xlsx格式的文件,多余的文件将被覆盖
      ...
      <div>
        <h1>森林相册</h1>
        <h2>
          <el-alert title="温馨提示：为本林区上传资料图片，单次上传不得超过9张" type="warning" :closable="false" style="font-size: small;">
          </el-alert>
        </h2>


        <el-upload ref="upload" v-model:file-list="fileList" :action="'http://127.0.0.1:5000/uploadForestImage'"
          list-type="picture-card" :on-preview="handlePictureCardPreview" :on-remove="handleRemove" :limit="9"
          :on-exceed="handleExceed" :on-success="handleSuccess" :on-error="handleError" :before-upload="beforeUpload"
          :auto-upload="false">
          <el-icon>
            <Plus />
          </el-icon>
        ...
        <el-button class="ml-3" type="success" @click="submitUploadForestImage" :loading="uploading"
          style="margin-top: 10px;">上传至林上鹰眼</el-button>
