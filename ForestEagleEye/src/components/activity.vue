      ...
      <div v-for="activity in filteredActivities" :key="activity.id" class="activity-card">
        ...
        <div>
          ...
          <div v-if="activeTab === 'enrollable'" class="activity-actions">
            <el-button style="width: 100%; justify-content: center;" plain type="success"
              @click="$router.push(`/enroll_activity/${activity.id}`)">报名</el-button>
          </div>
        </div>
      </div>
    </div>
